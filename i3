...
exec picom -e 1 -o 0 -i 0.95 --vsync -f --resize-damage 30
...
bindsym $mod+Mod1+q exec systemctl suspend
...
bindsym $mod+Print exec java -jar ~/screenshot.jar
bindsym Print exec sharenix-section -n
bindsym $mod+minus exec sharenix-window -n
...
bindsym Mod1+a exec --no-startup-id dmenu_run
